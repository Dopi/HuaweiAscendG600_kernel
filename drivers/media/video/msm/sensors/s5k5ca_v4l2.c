...
int32_t s5k5ca_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    if(false == s_ctrl->sensordata->standby_is_supported 
        || 0 == strcmp(s_ctrl->sensordata->sensor_name, ""))
    {
...
int32_t s5k5ca_set_mirror_mode(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (HW_MIRROR_AND_FLIP == get_hw_camera_mirror_type()) 
	{
		...
		rc = msm_camera_i2c_write_tbl(
			s_ctrl->sensor_i2c_client,
			&s5k5ca_mirror_flip_reg_config[0],
			ARRAY_SIZE(s5k5ca_mirror_flip_reg_config), 
			MSM_CAMERA_I2C_WORD_DATA);
	}
...
int32_t s5k5ca_write_init_settings(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc=0, i;
	...
	if(rc >= 0)
		rc = s5k5ca_set_mirror_mode(s_ctrl);	
...
static struct msm_sensor_fn_t s5k5ca_func_tbl = {
	...
	.sensor_write_init_settings = s5k5ca_write_init_settings,
};
